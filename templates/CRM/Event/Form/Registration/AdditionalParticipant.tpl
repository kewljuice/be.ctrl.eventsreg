...
<div class="eventsreg-container" id="eventsreg-confirm">
    ...
    <div class="eventsreg-block" id="eventsreg-participant-pre">
        {* open div class .eventsreg-title *}
        <div class="eventsreg-title"><h2>{ts domain='be.ctrl.eventsreg'}Participant information{/ts}</h2>
        </div>
        {* open div class .eventsreg-content *}
        <div class="eventsreg-content">
            <div class="crm-public-form-item crm-section custom_pre-section">
                {include file="CRM/UF/Form/CustomEventBlock.tpl" fields=$additionalCustomPre}
            </div>
        </div>
    ...
    <div class="crm-event-id-{$event.id} crm-block crm-event-additionalparticipant-form-block">
        ...
        {if $additionalCustomPost}
            {* open div class .eventsreg-block *}
            <div class="eventsreg-block" id="eventsreg-post">
                {* open div class .eventsreg-title *}
                <div class="eventsreg-title"><h2>{ts domain='be.ctrl.eventsreg'}Participant additional data{/ts}</h2>
                </div>
                {* open div class .eventsreg-content *}
                <div class="eventsreg-content">
                    <div class="crm-public-form-item crm-section custom_post-section">
                        {include file="CRM/UF/Form/CustomEventBlock.tpl" fields=$additionalCustomPost}
                    </div>
                </div>
                {* close div class .eventsreg-content *}
            </div>
        ...
        {if $priceSet}
            {* open div class .eventsreg-block *}
            <div class="eventsreg-block" id="eventsreg-price">
                {* open div class .eventsreg-title *}
                <div class="eventsreg-title"><h2>{ts domain='be.ctrl.eventsreg'}Price{/ts}</h2></div>
                <div>
                    {if $priceSet}
        ...
        <div class="eventsreg-button">
            <div id="crm-submit-buttons">
                {include file="CRM/common/formButtons.tpl"}
            </div>
        </div>
        {* close div class .eventsreg-button *}
...
{* close div class .eventsreg-container *}