    ...
    <div class="crm-event-id-{$event.id} crm-block crm-event-confirm-form-block">
        ...
        <div class="eventsreg-button" id="eventsreg-button-top">
            <div id="crm-submit-buttons" class="crm-submit-buttons">
                {include file="CRM/Event/Button/formButtons.tpl" location="top"}
            </div>
        </div>
        ...
        {if $paidEvent && !$isRequireApproval && !$isOnWaitlist}
            ...
            {* open div class .eventsreg-block *}
            <div class="eventsreg-block" id="eventsreg-eventfee">
                {* open div class .eventsreg-title *}
                <div class="eventsreg-title"><h2>{ts}{$event.fee_label}{/ts}</h2></div>
                {* open div class .eventsreg-content *}
                <div class="eventsreg-content">
                    {if $lineItem}
                    ...
                    {elseif $amounts || $amount == 0}
                        ...
                        {if $totalTaxAmount}
                            <div class="crm-section no-label total-amount-section">
                                <div class="content bold">{ts}Total Tax Amount{/ts}
                        ...
                        {if $totalAmount}
                            <div class="crm-section no-label total-amount-section">
                                <div class="content bold">{ts}Total Amount{/ts}
        ...
        {if $event.participant_role neq 'Attendee' and $defaultRole}
            {* open div class .eventsreg-block *}
            <div class="eventsreg-block" id="eventsreg-eventparticipant">
                {* open div class .eventsreg-title *}
                <div class="eventsreg-title"><h2>{ts}Participant Role{/ts}</h2></div>
                {* open div class .eventsreg-content *}
                <div class="eventsreg-content">
        ...
        {if $event.confirm_footer_text}
            ...
            <div class="eventsreg-footer">
                <div id="footer_text" class="crm-section event_confirm_footer-section">
                    <p>{$event.confirm_footer_text}</p>
                </div>
            </div>
            {* close div class .eventsreg-footer *}
        {/if}

        {* open div class .eventsreg-button *}
        <div class="eventsreg-button" id="eventsreg-button-bottom">
            <div id="crm-submit-buttons" class="crm-submit-buttons">
                {include file="CRM/Event/Button/formButtons.tpl" location="bottom"}
            </div>
        </div>
        {* close div class .eventsreg-button *}
