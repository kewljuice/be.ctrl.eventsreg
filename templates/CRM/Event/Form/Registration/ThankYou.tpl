...
<div class="crm-event-id-{$event.id} crm-block crm-event-thankyou-form-block">
    ...
    <div class="crm-section">
        ...
        <ul id="progressbar" class="nav nav-tabs">
            ...
            {/if}
            <li role="presentation" class="completed step-5">
                ...
                <div class="text-left">
                        ...
                        <div class="crm-group event_fees-group">
                            ...
                            {elseif $amount || $amount == 0}
                                ...
                                {if $totalAmount}
                                    <div class="crm-section no-label total-amount-section">
                                        <div class="content bold">{ts domain='be.ctrl.eventsreg'}Event Total{/ts}:&nbsp;&nbsp;{$totalAmount|crmMoney}</div>
                                        <div class="clear"></div>
                                    </div>
                            ...
                            {if $receive_date}
                                <div class="crm-section no-label receive_date-section">
                                    <div class="content bold">{ts domain='be.ctrl.eventsreg'}Transaction Date{/ts}
                                        : {$receive_date|crmDate}</div>
                                    <div class="clear"></div>
                                </div>
                            {/if}
                            {if $contributeMode ne 'notify' AND $trxn_id}
                                <div class="crm-section no-label trxn_id-section">
                                    <div class="content bold">{ts domain='be.ctrl.eventsreg'}Transaction #{/ts}
                                        : {$trxn_id}</div>
                                    <div class="clear"></div>
                                </div>
                            {/if}
                    ...
                    {elseif $participantInfo}
                        <div class="crm-group participantInfo-group">
                            <div class="header-dark">
                                {ts domain='be.ctrl.eventsreg'}Additional Participant Email(s){/ts}
                            </div>
                    ...
                    <div class="action-link section event_info_link-section">
                        <a href="{crmURL p='node/' q='reset=1' h=0}">&raquo; {ts 1=$event.event_title domain='be.ctrl.eventsreg'}Back to "%1" event information{/ts}</a>
                    </div>
